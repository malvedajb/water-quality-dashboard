window.statusForParam = function statusForParam(key, value) {
  ...
  if (key === "do_mgL") {
    ...
    if (value >= t.do_mgL.warnMin) return { label: "Moderate", color: "var(--warn)" };
    return { label: "Poor", color: "var(--bad)" };
  }

  if (key === "ph") {
    if (value >= t.ph.okMin && value <= t.ph.okMax) return { label: "Good", color: "var(--ok)" };
    if (value >= t.ph.warnMin && value <= t.ph.warnMax) return { label: "Moderate", color: "var(--warn)" };
    return { label: "Poor", color: "var(--bad)" };
  }
  ...
  if (key === "bod_mgL") {
    ...
    if (value <= t.bod_mgL.warnMax) return { label: "Moderate", color: "var(--warn)" };
    return { label: "Poor", color: "var(--bad)" };
  }
